...
function displayWord() {
  wordElement.innerHTML = `${selectedWord.split('').map(letter => `<span class="letter">${correctLetters.indexOf(letter) >= 0 ? letter : ''}</span>`).join('')}`;
...
function updateWrongLetters() {
  wrongLettersElement.innerHTML = `${wrongLetters.length > 0 ? '<p>Wrong letters</p>' : ''}${wrongLetters.map(letter => `<span>${letter}</span>`)}`;
...
buttonPlayAgain.addEventListener('click', async () => {
  ...
  await randomWord().then((res) => selectedWord = res).then(() => displayWord());
